...
#         self.logger.debug([self.dataset.vocab.convertToLabels(item) for item in surfaces])
...
class RelationOrderedLinker(OrderedLinker):
    ...
    def best_ranks(self, surfaces, qa_row, k, train):
        results = super(RelationOrderedLinker, self).best_ranks(surfaces, qa_row.sparql.relations, qa_row.question, k, train)
